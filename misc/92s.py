...
vad = webrtcvad.Vad(1)  # 1 = low aggression (1-3, where 3 is the most aggressive)

def is_speech(audio_bytes, sample_rate):
    # VAD requires 16-bit PCM audio and the frame size must be in multiples of 16000 Hz.
    ...
    for start in range(0, len(audio_data), frame_length):
        ...
        if stop > len(audio_data):
            break
        is_speech = vad.is_speech(audio_data[start:stop].tobytes(), sample_rate)
        if is_speech:
...
def main():
    ...
    pipe = pipeline(
        ...
        #device="mps" if args.device_id == "mps" else f"cuda",
        device=device,
        ...
        try:
            ...
            if not data_queue.empty():
                ...
                while not data_queue.empty():
                    data = data_queue.get()
                    if is_speech(data, sampling_rate):  # Only process if speech is detected
                        last_sample += data
